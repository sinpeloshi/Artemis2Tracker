...
app = FastAPI(title="Artemis 2 - NASA MOC (Mobile-Discernable)")
...
async def get_frontend():
    ...
    <head>
        ...
        <title>NASA MOC | Mobile Discernable</title>
        ...
        <style>
            ...
            @media (max-width: 800px) {
                ...
                #main-3d { order: 2; height: 50vh; min-height: 50vh; border-bottom: 2px solid var(--neon-blue); }
        ...
        <script>
            ...
            async function updateMOC() {
                try {
                    ...
                    // --- ESTO ES LO NUEVO: Conditional Mobile/Desktop Config ---
                    let earthSize, moonSize, orionSize, cameraEye;

                    // Check if mobile (Image 2 format)
                    const isMobile = window.innerWidth < 800;

                    if (isMobile) {
                        // Config para celular: EXAGERAMOS los tamaños para discernir
                        earthSize = 25; 
                        moonSize = 12; // Aumentado
                        orionSize = 10; // Aumentado
                        cameraEye = {x: 1.1, y: 1.1, z: 0.5}; // Cámara más cerca
                    } else {
                        // Config estándar para escritorio
                        earthSize = 15;
                        moonSize = 8;
                        orionSize = 6;
                        cameraEye = {x: 1.5, y: 1.5, z: 0.8}; // Vista lejana
                    }

                    const traces3D = [
                        { x: stX, y: stY, z: stZ, mode: 'markers', marker: {size: 1, color: '#444'}, type: 'scatter3d', hoverinfo: 'none' },
                        { x: [0], y: [0], z: [0], mode: 'markers', marker: { size: earthSize, color: '#00e5ff' }, type: 'scatter3d', name: 'Earth' },
                        { x: [d.moon.x], y: [d.moon.y], z: [d.moon.z], mode: 'markers', marker: { size: moonSize, color: '#aaa' }, type: 'scatter3d', name: 'Moon' },
                        { x: [d.orion.x], y: [d.orion.y], z: [d.orion.z], mode: 'markers', marker: { size: orionSize, color: '#ff3d00', symbol: 'diamond' }, type: 'scatter3d', name: 'Orion' },
                    ...
                    const layout3D = {
                        ...
                        scene: { 
                            ...
                            camera: { eye: cameraEye } // Usamos el ojo dinámico
                        }, showlegend: false
                    };
                    
                    Plotly.react('main-3d', traces3D, layout3D, {responsive: true, displayModeBar: false});

                    // GRÁFICO 2D (Responsive habilitado)
                    const layout2D = {
